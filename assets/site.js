...
/* ---- src/js/20-hero.js ---- */
/* ==========================================================================
   20-hero.js — the hero engine (CONTRACT §7), a port of the reference site's scroll
   ...
   Departure (CONTRACT §1): the reference site's headline beat (G.headline, t$ = 1.4)
   does not exist here; the timeline starts at what the reference site calls `g`.

   Drawing mechanics: the reference site tweens strokeDashoffset per element with GSAP
...
(function () {
  ...
  /* ---------- the reference site's constants ---------- */
  var K = {
  ...
  var CAM = {
    ...
    frameW: 1512, frameH: 900,          /* q, G  the reference site's Figma frame */
  ...
  /* the reference site tv(): strip the frame rects, normalise strokes, dash the .2 lines */
  ...
  /* the reference site tS(): collect the stroked / filled geometry of a group */
  ...
  /* the reference site tM(): the settled state */
  ...
  /* the reference site tD(), evaluated at time T for one segment {t0, dur, dir}.
     ...
     settles at t0 + dur (the reference site's beats assume that). */
  ...
  /* marquee: the reference site renders six identical groups (one live, five aria-hidden) */
  ...
  /* the .hero-walk button (the reference site's component `c`) */
  ...
  /* metrics numbers (the reference site tz/ek): count up over 1100ms, 160ms per item, cubic out */
  ...
  /* ======================================================================
     desktop engine (the reference site ec())
     ====================================================================== */
  ...
  function stopCam(stop, aspect, stageW) {                                           /* tp, from content/hero.json */
    ...
    if (stageW > 1024 && stop.bw) {
      /* nudge the room right of centre so its bbox clears the HUD column (the reference site's rooms sit .52–.63 across) */
  ...
  /* the camera as a pure function of the timeline time (the reference site tweens q; a callback-driven
  ...
  /* ---------- card / panels (the reference site X J K U te tg ti) ---------- */
  ...
  /* ---------- drawing render (the reference site's tD/tM effect, evaluated per frame) ---------- */
  ...
  /* ---------- the timeline (the reference site ty) ---------- */
  function onTick() {
    var tl = D.tl;
    if (!tl) return;
    camAt(tl.time(), D.cam);
  ...
  function build() {
    ...
    /* --- beat 1: blur, buildings draw in, words stagger, marquee fades (the reference site g … v) --- */
    ...
    /* --- trust out, vignette in, first dive (the reference site x … _) --- */
    var x = tl.duration();
    ...
    STOPS.forEach(function (stop, e) {
      ...
      if (e === 0) {
        ...
        /* the reference site: keyframes [{...mid, .45}, {...i, .55}] with the quintic across the whole tween (linear inside each keyframe) */
  ...
  /* ---------- scroll helpers (the reference site t_ tA R N O) ---------- */
  ...
  /* ---------- resize: kill + rebuild (the reference site tT) ---------- */
  function onResize() {
  ...
  function initDesktop() {
    ...
    /* trust words: split at runtime (the reference site renders <span><span class="trust-word">w</span> </span>) */
...
/* ==========================================================================
   21-hero-mobile.js — the reference site's phone experience for the hero (≤ 1024px).
   Port of the `e_` component in ref/hero_chunk.pretty.js (lines 19391–19680):
     · video screen: paragraph + walk hint fade in on mount (the reference site fades its
...
(function () {
  ...
  function init() {
    ...
    var ctas = Array.prototype.slice.call(root.querySelectorAll('[data-hero-m-cta]'));   /* Book a call + See what we do */
    ...
    if (reduce || !gsap) {
      ...
      ctas.forEach(function (c) { c.style.opacity = '1'; });
      if (walk) walk.style.opacity = '1';
    } else {
      gsap.set([para].concat(ctas, [walk]).filter(Boolean), { opacity: 0 });
      state.intro = gsap.timeline({ defaults: { ease: 'power2.out' } });
      if (para) state.intro.to(para, { opacity: 1, duration: .7 }, .15);
      if (ctas.length) state.intro.to(ctas, { opacity: 1, duration: .55 }, .5);
      if (walk) state.intro.to(walk, { opacity: 1, duration: .55 }, .5);
    }

    /* marquee: six groups like the reference site */
...
/* ---- src/js/30-quotes.js ---- */
/* ==========================================================================
   30-quotes.js — the home quotes section (the reference site #testimonials, ported from
   the React component in ref/hero_chunk.pretty.js ~19760–20100).

   Mechanism (the reference site's, time-based — it is NOT scroll-scrubbed):
   ...
   Ours gates the FIRST reveal on the section entering the viewport (the reference site
...
(function () {
  ...
  /* ---- split each quote into word spans (the reference site: text.split(' '), trailing space inside the span) ---- */
...
/* ==========================================================================
   31-features.js — the home features carousel (the reference site #features, ported from
   ...
   · ≤600px (the reference site's max-tp) the cards stack — CSS only, inline styles cleared.
   the reference site does not scroll-animate the cards; neither do we.
   ========================================================================== */
(function () {
  'use strict';

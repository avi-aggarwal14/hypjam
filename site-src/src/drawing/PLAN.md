...
`getBBox()` (Playwright) — geometry only; **not one path of the reference site's is copied**.
...
the reference site's hotel names because the engine depends on them; the *content* is hypjam's.
...
- Shells: no element enters the **interior** of any slot by more than 40 units. Lines *on* a slot
  edge are fine and expected (floor lines, ceiling lines). the reference site does exactly this: the balustrade
  top line at y=10094 stops at x=3795.5 and resumes at 11399.5; `#2-fade` draws the piece between.
- Keep a part file under ~700 KB. the reference site's entire drawing (3948 paths) is 750 KB.

---

## 2. The skeleton — measured from the reference site (canvas units; 1 unit ≈ 1 mm)

Canvas 44001 × 21918. the reference site's whole drawing occupies x 0→44000, y 5→21913. Ground line y = **21912.6**.
...
| cornice band 1 | 7003.6 → 7408.3 | 2199.8 → 15984.6 | profile: top line y=7003.6 full; the band is 2199.8→15984.6 down to y=7190.3, then steps in to 2305.1→15879.4 down to y=7408.3 (the reference site draws it as one closed outline) |
...
x0 = **3795.8** (fixed), **6814.2, 9832.7, 12851.1** (`#5-fade`). the reference site: outer semicircle spanning
...
**2.2d Ground-floor arches** (the reference site): semicircles of radius 1219, spring y=19649, apex 18430, jambs
...
the reference site draws A's side walls as pairs 168.6 apart (2408.2/2576.8, 15646.3/15814.7) on floors 2 and
1/ground, and single lines elsewhere. Keep exactly that: the pairs are the "wall thickness" the
eye reads as masonry.

### 2.4 Room slots (from CONTRACT §8 = the reference site's detail-group bboxes) and the reference site's fade bboxes

| stop | detail id | slot x, y, w, h | slot spans | the reference site fade bbox | the reference site counts fade / detail |
|---|---|---|---|---|---|
...
the reference site's interiors use opacity 0.4 for 24–49 % of elements and 0.2 for 0–23 %.
...
steps down to the street. Rooftop: a glazed **roof lantern** where the reference site has a plant box, and a
...
- **Top-floor arched window** (bays at x0 = 3795.8 fixed · 6814.2, 9832.7, 12851.1 in `#5-fade`):
  ...
  y≈4064, the reference site's apex) · inner opening `M x0+262 6562.6 V 4720 Q x0+822.5 3932 x0+1383 4720 V
...
- **Balustrade band** (10094→10960.2): outline as the reference site's shape (top line broken across slot 2);
...
do not draw through). No perspective — the reference site's rooms are flat elevations with a hint of depth
from overlap only.

### 4.6 Budgets and density
the reference site's interiors are 262–1157 elements. Targets per part are in §0. A room under ~300 elements
...
line in the reference site's shell (the upper-floor wall runs from 3119.6 down); leave that edge to room-3's
...
(the reference site had a round-arched door with columns; ours is a square industrial bay with a fanlight.)
...
- `python3 tools/check_drawing.py <file.svg> [...]` — validates a part (by its ids) or the
  ...
  warnings so the reference site's reference can be inspected (its structure and skeleton pass; its attribute
...
- `python3 tools/assemble_drawing.py [--strict] [--force]` — checks every part, merges them into
  ...
  the reference site's footer image shows exactly the exterior with its windows, so the fades are included).

...
Five mock hypjam screens live inside the hero HUD card, one per stop. the reference site's are Lottie files
...
Read CONTRACT.md §4, §7, §9, §10 first. Everything below is derived from it and from the reference site's
...
## 1. What the reference site's panels look like (measured)
...
the reference site's `tm[]` (engine line 92+): panel sizes `321×184`, `306×128`, `304×205`, `324×152`, `306×65`;
...
| mono data ("OCEAN BALLROOM \| 120 GUESTS") | monospace (the reference site loads `gtStandardMono`), 11px, uppercase, mute |
...
- **Width: 321px for all five.** (the reference site varies width per panel; we do not — the card stays 357 wide
...
| stop | panel | height | the reference site's | why |
|---|---|---|---|---|
...
- One root, class `pnl` + `pnl--<name>`, the two data attributes, `aria-hidden="true"`
  (the card's `aria-hidden` slot mirrors the reference site's; screen readers get the stop title/description only).
...
becomes active, `tl.pause()` when it leaves (the reference site's Lottie players do the same via visibility).
...
--pnl-mute:   #a1a09a                   labels, ids, meta   (the reference site's Lottie label grey)
...
| `.pnl-row` | `<div class="pnl-row"> …cells… </div>` | flex, gap 10, `min-height:28px`, hairline bottom (`.pnl-row--tall` = 34px like the reference site; `.pnl-row--bare` = no hairline) |
...
- Loop length **6–10s** (the reference site: 4.5–12.8s). Build one timeline, `repeat: -1`.
...
- **Crossfade between beats: 0.28s** (the reference site's `eh`). Outgoing beat: opacity → 0 and y → −6;

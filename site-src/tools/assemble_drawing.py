...
and write assets/img/hq-footer.svg (the exterior only, for the footer's small drawing slot).
  ...
  * XML prologs, comments, and every id except the reserved group ids are stripped; attributes are
    re-serialised in a fixed order (geometry first, then stroke attributes), exactly like the reference site's file
  * output structure:
  ...
  * hq-footer.svg = fixed-A + fixed-B + the five fades (the reference site's footer image is exactly the exterior with
